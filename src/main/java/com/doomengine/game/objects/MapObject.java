...
import static com.doomengine.game.objects.Actions.*;
...
public abstract class MapObject {
    ...
    private void updateAttackingState(MapObject player, double distance, boolean canSeePlayer) {
        ...
        if (target != null) {
            this.angle = Math.toDegrees(Math.atan2(target.pos.y() - this.pos.y(), target.pos.x() - this.pos.x()));
        }
    ...
    private boolean isInNonAttackState() {
        ...
        return switch (type) {
            case MT_POSSESSED ->
                    isInRange(currentStateNum.ordinal(), StateNum.S_POSS_ATK1.ordinal(), StateNum.S_POSS_ATK3.ordinal());
            case MT_SHOTGUY ->
                    isInRange(currentStateNum.ordinal(), StateNum.S_SPOS_ATK1.ordinal(), StateNum.S_SPOS_ATK3.ordinal());
            case MT_TROOP ->
                    isInRange(currentStateNum.ordinal(), StateNum.S_TROO_ATK1.ordinal(), StateNum.S_TROO_ATK3.ordinal()) &&
                            isInRange(currentStateNum.ordinal(), StateNum.S_TROO_MISS1.ordinal(), StateNum.S_TROO_MISS3.ordinal());
            case MT_SERGEANT ->
                    isInRange(currentStateNum.ordinal(), StateNum.S_SARG_ATK1.ordinal(), StateNum.S_SARG_ATK4.ordinal());
            default -> true;
        };
    }
    ...
    private StateNum getRunState() {
        return switch (type) {
            case MT_POSSESSED -> StateNum.S_POSS_RUN1;
            case MT_SHOTGUY -> StateNum.S_SPOS_RUN1;
            case MT_CHAINGUY -> StateNum.S_CPOS_RUN1;
            case MT_TROOP -> StateNum.S_TROO_RUN1;
            case MT_SERGEANT, MT_SHADOWS -> StateNum.S_SARG_RUN1;
            case MT_HEAD -> StateNum.S_HEAD_RUN1;
            case MT_SKULL -> StateNum.S_SKULL_RUN1;
            case MT_KNIGHT -> StateNum.S_BOS2_RUN1;
            case MT_BRUISER -> StateNum.S_BOSS_RUN1;
            case MT_FATSO -> StateNum.S_BOSS_RUN1; // Use Baron for now
            case MT_BABY -> StateNum.S_HEAD_RUN1; // Use Cacodemon for now
            case MT_PAIN -> StateNum.S_HEAD_RUN1; // Use Cacodemon for now
            case MT_SPIDER -> StateNum.S_SPID_RUN1;
            case MT_CYBORG -> StateNum.S_CYBER_RUN1;
            case MT_VILE -> StateNum.S_HEAD_RUN1; // Use Cacodemon for now
            case MT_WOLFSS -> StateNum.S_POSS_RUN1; // Use zombieman
            default -> null;
        };
    }
    ...
    private double getAttackRange() {
        return switch (type) {
            ...
            case MT_TROOP ->      // Imp - fireball
                    448.0;   // Demon - melee only
            case MT_SERGEANT, MT_SHADOWS ->    // Spectre - melee only
                    64.0;
    ...
    private void updateProjectileMovement() {
        ...
        // Move projectile
        Vector2D newPos = pos.add(velocity.multiply(deltaTime));
