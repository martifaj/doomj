...
public class VisSprite {
    public MapObject mo;           // Reference to the original game object
    public int screenX1, screenX2; // Projected screen X boundaries (inclusive)
    public int screenY1, screenY2; // Projected screen Y boundaries (full sprite extent before world clipping)
    public double scale;           // For depth sorting and comparison (larger scale = closer)
    public double depth;           // Actual depth value (e.g., distance to sprite center)
    public BufferedImage image;    // The sprite image to draw (already scaled by AssetData)
    public boolean fullBright;
    public int lightLevelInt;      // Original sector light level (0-255) for the sprite
